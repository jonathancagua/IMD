...
{
	int retval;
	retval = del_timer(&my_timer); /* Is timer still active (1) or no (0) */
	if (retval)
...
module_init(my_init);
module_exit(my_exit);
MODULE_AUTHOR("Daniel Lara <sirdeniel3@gmail.com>");
MODULE_DESCRIPTION("Standard timer example from book");
MODULE_LICENSE("GPL");